...
class tool_lp_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_data_for_user_competency_summary_in_plan() {
        ...
        // Test external data_for_user_competency_summary_in_plan with display rating off.
        // Template level.
        external::set_display_rating_for_template($tpl->get('id'), 0);
        // Login with user.
        $this->setUser($this->user);

        $summary = external::data_for_user_competency_summary_in_plan($c1->get('id'), $plan->get('id'));
        $this->assertEquals('-', $summary->usercompetencysummary->usercompetency->gradename);
        $this->assertEquals('-', $summary->usercompetencysummary->usercompetency->proficiencyname);
        $this->assertEmpty($summary->usercompetencysummary->evidence);
        // Plan level.
        $this->setUser($this->creator);
        external::set_display_rating_for_plan($plan->get('id'), 1);
        $this->setUser($this->user);
        ...
        $this->setUser($this->creator);
        external::reset_display_rating_for_plan($plan->get('id'));
        $this->setUser($this->user);
    ...
    public function test_set_display_rating_for_template() {
        global $CFG;
        ...
        $plan = $lpg->create_plan(array('userid' => $this->user->id, 'templateid' => $tpl->get('id'), 'name' => 'Evil',
            'status' => \core_competency\plan::STATUS_ACTIVE));
        external::set_display_rating_for_template($tpl->get('id'), 0);
        $shoulddisplaytpl = \tool_lp\api::has_to_display_rating_for_template($tpl->get('id'));
        $shoulddisplayplan = \tool_lp\api::has_to_display_rating_for_plan($plan->get('id'));
        $this->assertFalse($shoulddisplaytpl);
        $this->assertFalse($shoulddisplayplan);
        // Has to display for manager.
        $displayratingmanager = \tool_lp\api::has_to_display_rating($plan->get('id'));
        $this->assertTrue($displayratingmanager);
        // Has to display rating for student.
        $this->setUser($this->user);
        $displayratingstudent = \tool_lp\api::has_to_display_rating($plan->get('id'));
        $this->assertFalse($displayratingstudent);

        // Set display rating to true.
        $this->setUser($this->creator);
        external::set_display_rating_for_template($tpl->get('id'), 1);
        $shoulddisplaytpl = \tool_lp\api::has_to_display_rating_for_template($tpl->get('id'));
        $shoulddisplayplan = \tool_lp\api::has_to_display_rating_for_plan($plan->get('id'));
        $this->assertTrue($shoulddisplaytpl);
        $this->assertTrue($shoulddisplayplan);

        // Has to display rating for student.
        $this->setUser($this->user);
        $displayratingstudent = \tool_lp\api::has_to_display_rating($plan->get('id'));
        $this->assertTrue($displayratingstudent);
    }

    public function test_set_display_rating_for_plan() {
        global $CFG;
        ...
        $plan = $lpg->create_plan(array('userid' => $this->user->id, 'templateid' => $tpl->get('id'), 'name' => 'Evil',
            'status' => \core_competency\plan::STATUS_ACTIVE));
        // Set display rating off in plan level.
        external::set_display_rating_for_plan($plan->get('id'), 0);
        $shoulddisplayplan = \tool_lp\api::has_to_display_rating_for_plan($plan->get('id'));
        $this->assertFalse($shoulddisplayplan);
        // Has to display for manager.
        $displayratingmanager = \tool_lp\api::has_to_display_rating($plan->get('id'));
        ...
        $displayratingstudent = \tool_lp\api::has_to_display_rating($plan->get('id'));
        $this->assertFalse($displayratingstudent);
        // Check that dispay rating in level plan override template level (plan= off , template = on).
        $this->setUser($this->creator);
        external::set_display_rating_for_template($tpl->get('id'), 1);
        ...
        $displayratingstudent = \tool_lp\api::has_to_display_rating($plan->get('id'));
        $this->assertFalse($displayratingstudent);
        // Check that dispay rating in level plan override template level (plan= on , template = off).
        $this->setUser($this->creator);
        external::set_display_rating_for_template($tpl->get('id'), 0);
        external::set_display_rating_for_plan($plan->get('id'), 1);
    ...
    public function test_reset_display_rating_for_template() {
        global $CFG;
        ...
        // Set display rating on in template level.
        external::set_display_rating_for_template($tpl->get('id'), 1);
        // Check there is no plan associated to the template with his own display rating setting.
        $planwithdisplayrating = external::template_has_plan_with_display_rating($tpl->get('id'));
        $this->assertFalse($planwithdisplayrating);

        // Set display rating off in plan level.
        external::set_display_rating_for_plan($plan->get('id'), 0);
        // Check there is a plan associated to the template with his own display rating setting.
        $planwithdisplayrating = external::template_has_plan_with_display_rating($tpl->get('id'));
        $this->assertTrue($planwithdisplayrating);
        // Check there is a plan associated to the template with a different display rating setting.
        $planwithdisplayrating = external::template_has_plan_with_display_rating($tpl->get('id'), true);
        $this->assertTrue($planwithdisplayrating);

        // Reset display rating for plan.
        external::reset_display_rating_for_template($tpl->get('id'));
        // Check there is no plan associated to the template with his own display rating setting.
        $planwithdisplayrating = external::template_has_plan_with_display_rating($tpl->get('id'));
        $this->assertFalse($planwithdisplayrating);
    }

    public function test_reset_display_rating_for_plan() {
        global $CFG;
    ...
    public function test_can_reset_display_rating_for_plan() {
        global $CFG;
        ...
        // Create plan linked to a template.
        $plan1 = $lpg->create_plan(array('userid' => $this->user->id, 'templateid' => $tpl->get('id'), 'name' => 'Evil',
            'status' => \core_competency\plan::STATUS_ACTIVE));

        // Set display rating on in template level.
        external::set_display_rating_for_template($tpl->get('id'), 1);

        // Plan does not have a display rating setting. Reset is not possible.
        $canresetdisplayrating = external::can_reset_display_rating_for_plan($plan1->get('id'));
        $this->assertfalse($canresetdisplayrating);

        // Set display rating off in plan level.
        external::set_display_rating_for_plan($plan1->get('id'), 0);
        // Plan associated to the template with his own display rating setting. Reset is possible.
        $canresetdisplayrating = external::can_reset_display_rating_for_plan($plan1->get('id'));
        $this->asserttrue($canresetdisplayrating);

        // Create plan not linked to a template.
        $plan2 = $lpg->create_plan(array('userid' => $this->user->id, 'templateid' => null, 'name' => 'Alone',
            'status' => \core_competency\plan::STATUS_ACTIVE));
        // Set display rating off in plan 2 level.
        external::set_display_rating_for_plan($plan2->get('id'), 0);
        // Reset is not possible.
        $canresetdisplayrating = external::can_reset_display_rating_for_plan($plan2->get('id'));
        $this->assertfalse($canresetdisplayrating);
    }
